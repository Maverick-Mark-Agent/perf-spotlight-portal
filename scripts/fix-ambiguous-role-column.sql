-- =====================================================
-- FIX AMBIGUOUS ROLE COLUMN ERROR
-- =====================================================
-- The function has an ambiguous reference to "role" column
-- Need to specify table alias (uwa.role vs cr.role)
-- =====================================================
...
BEGIN
  ...
  ELSE
    ...
    -- FIX: Specify uwa.role explicitly to avoid ambiguity
    RETURN QUERY
    SELECT
      cr.workspace_id,
      uwa.workspace_name,
      uwa.role,  -- Explicitly use uwa.role
      COUNT(DISTINCT cl.id) AS leads_count,
      COUNT(DISTINCT cl.id) FILTER (WHERE cl.pipeline_stage = 'won') AS won_leads_count
    ...
    GROUP BY cr.workspace_id, uwa.workspace_name, uwa.role  -- Explicitly use uwa.role in GROUP BY
