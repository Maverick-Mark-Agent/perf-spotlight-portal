...
async function analyzeBurntDomainsAllWorkspaces() {
  ...
  console.log('Fetching all email accounts from database (no status filter)...\n');

  // Fetch ALL accounts without status filter
  ...
  // Filter to only include accounts with some activity
  const activeAccounts = allAccounts.filter(acc => {
    const emailsSent = acc.emails_sent_count || 0;
    return emailsSent > 0; // Only include accounts that have sent at least 1 email
  });

  console.log(`📊 Filtered to ${activeAccounts.length} accounts with email activity\n`);
  ...
  console.log(`Found ${domainMap.size} unique domains with activity\n`);
  ...
  console.log(`Total accounts with activity: ${activeAccounts.length}`);
  console.log('');
