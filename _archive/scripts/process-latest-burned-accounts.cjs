...
/**
 * Process the latest burned accounts CSV (Nov 24)
 */
...
/**
 * Parse the CSV and extract unique email addresses
 */
    ...
    if (fields.length > 4 && fields[4].trim()) {
      ...
      if (emailMatches) {
        ...
        emailMatches.forEach(email => {
          ...
          // Filter out third-party emails
          if (cleanEmail &&
              !cleanEmail.includes('opensend') &&
              !cleanEmail.includes('infonycs') &&
              !cleanEmail.includes('visionair') &&
              !cleanEmail.includes('fllending') &&
              !cleanEmail.includes('trysalessociety') &&
              !cleanEmail.includes('securifyprotection') &&
              !cleanEmail.includes('findymail') &&
              !cleanEmail.includes('coachpronext') &&
              !cleanEmail.includes('withwemdev') &&
              !cleanEmail.includes('infopstg') &&
              !cleanEmail.includes('getlessstress') &&
              !cleanEmail.includes('gigabrain') &&
              !cleanEmail.includes('ssrecruiting') &&
              !cleanEmail.includes('homepolicyinsurance') &&
              !cleanEmail.includes('streetsmart')) {
...
async function main() {
  ...
  const burnedEmails = parseCSV(inputFile);
  ...
  // Generate CSV
  const outputFile = '/Users/mac/Downloads/perf-spotlight-portal/burned-accounts-with-tags-latest.csv';
  ...
  results.forEach(r => {
    ...
    if (r.workspace !== 'NOT_FOUND' && r.workspace !== 'N/A') {
      workspaceCounts[r.workspace] = (workspaceCounts[r.workspace] || 0) + 1;
    }
  });
