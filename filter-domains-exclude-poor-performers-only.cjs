...
console.log('📋 Filtering domains - excluding only poor performers (avg reply rate <0.4% with volume)...\n');
...
// Write full data CSV
fs.writeFileSync('DOMAINS-EXCLUDING-POOR-PERFORMERS.csv', goodDomains.join('\n'));
...
fs.writeFileSync('DOMAINS-EXCLUDING-POOR-PERFORMERS-LIST-ONLY.csv', listOnly.join('\n'));

console.log('✅ FILTERING COMPLETE\n');
console.log('='.repeat(70));
console.log(`Total domains kept: ${goodDomains.length - 1}`);
console.log(`Total domains excluded: ${excludedDomains.length}`);
console.log('');
console.log('📋 Excluded domains (poor performers with volume):');
excludedDomains.slice(0, 10).forEach(d => {
  console.log(`   - ${d.domain} (${d.classification}, ${d.totalSent} sent, ${d.avgReplyRate}% avg rate)`);
});
if (excludedDomains.length > 10) {
  console.log(`   ... and ${excludedDomains.length - 10} more`);
}
console.log('');
console.log('✅ Generated files:');
console.log('   - DOMAINS-EXCLUDING-POOR-PERFORMERS.csv (full data)');
console.log('   - DOMAINS-EXCLUDING-POOR-PERFORMERS-LIST-ONLY.csv (domain list)');
console.log('');
