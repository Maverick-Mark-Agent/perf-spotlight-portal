 ...
 * - Standard: <$900k (for all non-HNW contacts)
 * - High Net Worth (TX only): ≥$900k (Texas contacts move to Kirk Hodgson)
 *
 * Note: HNW threshold is $900k. Texas HNW contacts are MOVED (not duplicated) to Kirk Hodgson.
 */
function checkValueCriteria(homeValue: number, state: string): {
  ...
  const HNW_THRESHOLD = 900000; // $900k+ for HNW routing to Kirk Hodgson

  if (isTexas && homeValue >= HNW_THRESHOLD) {
    return { meetsStandard: false, isHNW: true };
  }

  return { meetsStandard: homeValue < HNW_THRESHOLD, isHNW: false };
}
...
function processContact(
  ...
  // Validate and log home value parsing
  if (homeValue > 0 && homeValue < 100000000) {
    // Valid home value range
    if (propertyState === 'TX' && homeValue >= 900000) {
      console.log(`🎯 HNW Candidate: ${firstName} ${lastName}, ${row['Property City']?.trim()} TX, $${homeValue.toLocaleString()}`);
    }
  } else if (homeValue >= 100000000) {
    console.warn(`⚠️  Suspicious home value: $${homeValue.toLocaleString()} for ${email} (may be parsing error)`);
  }
...
serve(async (req) => {
  ...
  try {
    ...
    // =====================================================
    // KIRK HODGSON HNW ROUTING: MOVE (not duplicate) Texas HNW contacts
    // =====================================================
    // If this is a Texas agency, split contacts between original workspace and Kirk Hodgson
    ...
    if (isTexasAgency) {
      ...
      if (kirkWorkspace) {
        // Split contacts: HNW TX → Kirk, everything else → original workspace
        ...
        if (hnwTexasContacts.length > 0) {
          ...
          // Create contacts for Kirk Hodgson (MOVE, not duplicate)
          kirkContacts = hnwTexasContacts.map(contact => {
            const { upload_batch_id, ...contactData } = contact;
            return {
              ...contactData,
              upload_batch_id: crypto.randomUUID(), // New batch ID for Kirk
        ...
        console.warn('Kirk Hodgson workspace not found - skipping HNW routing');
      }
    }
    ...
    // Insert HNW contacts to Kirk Hodgson (if any)
    if (kirkContacts.length > 0) {
      ...
      if (kirkInsertError) {
        console.error('Warning: Failed to route HNW contacts to Kirk Hodgson:', kirkInsertError);
        // Don't fail the whole upload, just log the error
        kirkRoutingCount = 0; // Reset count if insert failed
      } else {
    ...
    const responseMessage = kirkRoutingCount > 0
      ? `Processed ${totalContacts} contacts for ${clientName}: ${standardContacts.length} added to ${clientName}, ${kirkRoutingCount} HNW Texas contacts moved to Kirk Hodgson, ${filteredOut} filtered out, ${zipsInserted} ZIP codes added to territory map.`
      : `Processed ${totalContacts} contacts: ${readyForVerification} ready for verification, ${filteredOut} filtered out, ${zipsInserted} ZIP codes added to territory map`;
