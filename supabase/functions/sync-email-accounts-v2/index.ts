/**
 * Sync Email Accounts Edge Function (v2 - Workspace-Specific API Keys Only)
 *
 * Fetches email accounts from Email Bison API using workspace-specific keys
 * and syncs to email_accounts_raw table
 *
 * KEY CHANGE: No super admin fallback - requires workspace-specific API key
 *
 * Usage:
 *   POST /functions/v1/sync-email-accounts-v2
 *   Body: { "workspace_name": "John Roberts" } // Optional - syncs all if omitted
 */

import { serve } from 'https://deno.land/std@0.168.0/http/server.ts';
import { createClient } from 'https://esm.sh/@supabase/supabase-js@2';
import { fetchAllSenderEmails } from '../_shared/workspaceApiClient.ts';
    ...
    for (const workspace of workspaces) {
      ...
      try {
        ...
        if (!workspace.bison_api_key) {
          throw new Error(
            `Workspace '${workspace.workspace_name}' does not have an API key configured. ` +
            `Please generate a workspace-specific API key in Email Bison dashboard.`
          );
        }

        // Fetch all sender emails using the shared API client
        const senderEmails = await fetchAllSenderEmails(
          workspace.workspace_name,
          'sync-email-accounts-v2'
        );

        console.log(`Fetched ${senderEmails.length} accounts from ${workspace.workspace_name}`);

        // Normalize instance name for database constraint
        ...
        results.push({
          workspace_name: workspace.workspace_name,
          accounts_synced: senderEmails.length,
          status: 'success',
          duration_ms: Date.now() - wsStartTime,
        });

        console.log(`✅ Synced ${senderEmails.length} accounts for ${workspace.workspace_name}`);
