...
echo "============================================"
echo "  Update Leads with Reply UUIDs"
echo "  (Airtable URL Pattern)"
echo "============================================"
echo ""
...
echo -e "${YELLOW}Fetching reply UUIDs...${NC}"
TEMP_FILE="/tmp/reply-uuids-$$.txt"
> "$TEMP_FILE"
...
while IFS='|' read -r REPLY_UUID LEAD_ID; do
  ...
  PAYLOAD=$(jq -n \
    --arg uuid "$REPLY_UUID" \
    --arg url "$CONV_URL" \
    '{bison_reply_uuid: $uuid, bison_conversation_url: $url}')

  # Update by bison_lead_id
  curl -s -X PATCH "${SUPABASE_URL}/rest/v1/client_leads?workspace_name=eq.Devin%20Hodo&bison_lead_id=eq.${LEAD_ID}" \
  ...
  ((UPDATED++))
  echo "  ✅ Lead ${LEAD_ID} → UUID ${REPLY_UUID}"
