...
function extractDomainsFromCSV() {
  console.log('📋 Reading Google Zapmail domains from Excel CSV...\n');

  const csvContent = fs.readFileSync('./google-zapmail-domains-from-excel.csv', 'utf8');
  ...
  console.log(`✅ Found ${domainData.length} Google Zapmail domains in Excel file\n`);
  return domainData;
}

async function analyzeGoogleZapmailDomains() {
  const domainData = extractDomainsFromCSV();

  console.log('🔍 Analyzing each Google Zapmail domain from Supabase database...\n');
  ...
  // Print summary
  console.log('\n' + '='.repeat(70));
  console.log('📊 GOOGLE ZAPMAIL DOMAINS ANALYSIS');
...
function generateReports(results) {
  ...
  fs.writeFileSync('GOOGLE-ZAPMAIL-COMPLETE-SUMMARY.csv', summaryCSV.join('\n'));
  ...
  fs.writeFileSync('GOOGLE-ZAPMAIL-BURNT-DOMAINS.csv', burntCSV.join('\n'));
  ...
  fs.writeFileSync('GOOGLE-ZAPMAIL-HEALTHY-DOMAINS.csv', healthyCSV.join('\n'));
  ...
  fs.writeFileSync('GOOGLE-ZAPMAIL-DETAILED-MAILBOXES.csv', detailedCSV.join('\n'));
  ...
  if (results.notInDB.length > 0) {
    ...
    fs.writeFileSync('GOOGLE-ZAPMAIL-NOT-IN-DATABASE.csv', notInDBCSV.join('\n'));
  }

  console.log('✅ Generated CSV files:');
  console.log(`   - GOOGLE-ZAPMAIL-COMPLETE-SUMMARY.csv (all domains overview)`);
  console.log(`   - GOOGLE-ZAPMAIL-BURNT-DOMAINS.csv (${results.burnt.length} burnt domains)`);
  console.log(`   - GOOGLE-ZAPMAIL-HEALTHY-DOMAINS.csv (${results.healthy.length} healthy domains)`);
  console.log(`   - GOOGLE-ZAPMAIL-DETAILED-MAILBOXES.csv (mailbox-level breakdown)`);
  if (results.notInDB.length > 0) {
    console.log(`   - GOOGLE-ZAPMAIL-NOT-IN-DATABASE.csv (${results.notInDB.length} domains not found)`);
  }
  console.log('');
}

analyzeGoogleZapmailDomains().catch(console.error);
